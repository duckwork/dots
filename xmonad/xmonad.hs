...
ws1 = "home" -- beginning screen
ws2 = "surf" -- web browser
ws3 = "edit" -- coding + writing
ws4 = "docs" -- gui editors (libreoffice, gimp, etc.)
ws5 = "view" -- media
-- }}}
myLayoutHook = -- {{{
  avoidStruts
...
myLogHook h = do -- {{{ ( & dynamicLogWithPP config
  ...
  let checkCopy ws
        | ws `elem` copies = xmobarColor (red myCS) "" $ ws
        | otherwise        = xmobarColor (yellow myCS) "" $ ws
      mkLengthL n xs
        | length xs < n = replicate (n - length xs) ' ' ++ xs
...
myManageHook = -- {{{
  ...
  <+> (composeAll . concat $ -- {{{
    ...
    , [(className =? x <||> title =? x <||> resource =? x)
                      --> doShiftAndGo ws1 | x <- ws1Shifts]
    , [(className =? x <||> title =? x <||> resource =? x)
                      --> doShiftAndGo ws2 | x <- ws2Shifts]
    , [(className =? x <||> title =? x <||> resource =? x)
                      --> doShiftAndGo ws3 | x <- ws3Shifts]
    , [(className =? x <||> title =? x <||> resource =? x)
                      --> doShiftAndGo ws4 | x <- ws4Shifts]
    , [(className =? x <||> title =? x <||> resource =? x)
                      --> doShiftAndGo ws5 | x <- ws5Shifts]
    ]) -- }}}
    where
      -- doShiftAndGo = doF . liftM2 (.) W.greedyView W.shift
      doShiftAndGo = doShift
      myFloats =  -- float these windows
        [ "MPlayer"
        , "xmessage"
        ]
      myIgnores =  -- ignore these windows
        [ "desktop_window"
        , "kdesktop"
        ]
      ws1Shifts = []
      ws2Shifts =  -- web browsers
        [ ".uzbl-core-wrapped"
        , "Navigator" , "Firefox"
        , "surf", "Surf"
        , "Chromium"
        ]
      ws3Shifts =  -- text wrangling
        [ "vim"
        , "gvim", "Gvim"
        ]
      ws4Shifts = []
...
myHandleEventHook = -- {{{
  ...
  <+> docksEventHook
-- }}}
myStartupHook = return ()  -- empty for now
-- }}}
-- {{{ Key & Mouse bindings
...
myKeymap = \c -> mkKeymap c $
  ...
  , ("M-C-'",             switchLayer)  -- TODO: change to toggle
  , ("M-h",               declareMsg "goL")
  ...
  , ("M-<Space>",  spawn $ "exe=`yeganesh -x -p exe -- "
                     ++ myDmenuOpts
                     ++ "` && eval \"exec $exe\"")
